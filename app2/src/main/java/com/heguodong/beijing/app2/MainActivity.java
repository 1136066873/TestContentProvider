package com.heguodong.beijing.app2;
...
public class MainActivity extends AppCompatActivity implements AlarmClockContentListener, View.OnClickListener {
    ...
    private static final String TAG = "heguodong_app2";
    @Override
    ...
    protected void onResume() {
        super.onResume();
        //先查一遍
        Log. e(TAG, "------------------- app2 初始化时，首先查询一下数据库中数据 -------------------" );
    ...
    private void addData() {
        ContentValues values1 = new AlarmClockContentValuesBuilder()
                ...
                .datetime(20200328)
                .frequency(6)
                .remarks("备注信息：这是在 app2 中添加的数据")
                .build();

        AlarmClockResolver.getInstance(this).insert(values1);
        //AlarmClockResolver.getInstance(this).update("2",values1);
        //AlarmClockResolver.getInstance(this).delete("2");
    }
    ...
    public void onAlarmClockContentChange() {
        Log.e(TAG,"\n\n\n\n\n\n.\n\n\n\n\n\n");
        Log.e(TAG,"---------------------- 在 app2 中监测到数据发生变化，接下来重新遍历数据 ----------------------");
        queryMyDatabase(AlarmClockResolver.getInstance(this).query());
        Log.e(TAG,"---------------------- 数据发生变化后，在 app2 中遍历数据结束 ----------------------" );
    }
